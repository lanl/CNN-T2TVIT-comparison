...
class CNN_Interpretability_Module(nn.Module):
	def __init__(self, 
		...
		self.inConv = nn.Conv2d(in_channels = C,
								   ...
								   bias = True)
		self.inNorm = nn.BatchNorm2d(features)
		...
		for i in range(self.depth - 1):
			...
			self.interpNorms.append(nn.BatchNorm2d(features))
...
class CNN_Reduction_Module(nn.Module):
	def __init__(self, 
		...
		self.inNorm = nn.BatchNorm2d(features)
		self.inActivation = act_layer()
		W, H, _ = conv2d_shape(w=W, h=H, k=self.kernel, s_w=stride, s_h=stride, p_w=stride, p_h=stride)
		self.depth += 1 
		...
		while (W>W_lim or H>H_lim):
			...
			self.reductionNorms.append(nn.BatchNorm2d(features))
